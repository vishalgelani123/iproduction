                    ...
                    <table id="datatable" class="table table-striped">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th class="w-40">@lang('index.description')</th>
                                <th class="w-15">@lang('index.added_by')</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($obj as $value)
                                <tr>
                                    ...
                                    <td>{{ $value->name }}</td>
                                    <td>{{ safe($value->description) }}</td>
                                    <td>{{ $value->addedBy->name }}</td>
                                    <td class="ir_txt_center">
                                        @if (routePermission('expense-category.edit'))
                                            <a href="{{ url('expense-category') }}/{{ encrypt_decrypt($value->id, 'encrypt') }}/edit"
                                                class="button-success" data-bs-toggle="tooltip" data-bs-placement="top"
                                                title="@lang('index.edit')"><i class="fa fa-edit tiny-icon"></i></a>
                                        @endif
                                        @if (routePermission('expense-category.delete'))
                                            <a href="#" class="delete button-danger"
                                                ...
                                                <form action="{{ route('expense-category.destroy', $value->id) }}"
