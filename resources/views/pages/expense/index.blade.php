                    ...
                    <table id="datatable" class="table table-striped">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th class="w-20">@lang('index.responsible_person')</th>
                                <th class="w-15 ir_txt_center">@lang('index.actions')</th>
                            </tr>
                        </thead>
                        <tbody>
                            @if ($obj && !empty($obj))
                            ...
                            @foreach ($obj as $value)
                                <tr>
                                    ...
                                    <td>{{ $value->responsibleBy->name }}</td>
                                    <td class="ir_txt_center">
                                        @if (routePermission('expense.edit'))
                                            <a href="{{ url('expense') }}/{{ encrypt_decrypt($value->id, 'encrypt') }}/edit" class="button-success"
                                                data-bs-toggle="tooltip" data-bs-placement="top"
                                                title="@lang('index.edit')"><i class="fa fa-edit tiny-icon"></i></a>
                                        @endif
                                        @if (routePermission('expense.delete'))
                                            <a href="#" class="delete button-danger"
                                                ...
                                                <form action="{{ route('expense.destroy', $value->id) }}"
