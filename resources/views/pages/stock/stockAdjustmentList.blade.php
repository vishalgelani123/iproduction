                    ...
                    <table id="datatable" class="table table-striped">
                        <thead>
                            <tr>
                                ...
                                <th class="width_13_p">@lang('index.raw_material')(@lang('index.code'))</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($obj as $value)
                                <tr>
                                    ...
                                    <td>{{ $value->rawMaterials->name }}({{ $value->rawMaterials->code }})</td>
                                    <td>{{ ucwords($value->type) }}</td>
                                    <td>{{ $value->quantity }} {{ getUnitByRMID($value->rm_id) }}</td>                                    
                                    <td class="ir_txt_center">
                                        @if (routePermission('stock-adjust.edit'))
                                            <a href="{{ route('stockAdjustEdit', encrypt_decrypt($value->id, 'encrypt')) }}"
