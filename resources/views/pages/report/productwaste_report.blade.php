...
<section class="main-content-wrapper">
    ...
    <div class="box-wrapper">
        {{Form::open(['route'=>'rmwaste-report', 'id' => "expense_form", 'method'=>'get'])}}
        <div class="row">
                ...
                <table id="datatable" class="table table-striped">
                    <thead>
                        <tr>
                            ...
                            <th class="op_width_10_p">@lang('index.loss_amount')</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        if (!empty($obj)) {
                            ...
                            foreach ($obj as $value) {
                                $total += $value->total_loss;
                        ?>
                                <tr>
                                    <td class="text-start">{{ $i-- }}</td>
                                    <td>{{ $value->reference_no }}</td>
                                    <td>{{ getDateFormat($value->date) }}</td>
                                    <td>{{ getAmtCustom($value->total_loss) }}</td>
                                </tr>
                        <?php
                            }
                        }
                    ...
                    <tfoot>
                        <tr>
                            <th></th>
                            <th></th>
                            <th class="text-end">@lang('index.total_loss')=</th>
