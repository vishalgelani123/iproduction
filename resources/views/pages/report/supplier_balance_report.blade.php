...
@section('content')
    ...
    <section class="main-content-wrapper">
        ...
        <div class="box-wrapper">
            {{ Form::open(['route' => 'supplier-balance-report', 'id' => 'attendance_form', 'method' => 'get']) }}
            <div class="row">
                <div class="col-sm-12 mb-3 col-md-4 col-lg-2">
                    <div class="form-group">
                        <select class="form-control select2 op_width_100_p" id="type" name="type">
                            <option value="All" {{ $type == 'All' ? 'selected="selected"' : '' }}>@lang('index.all')
                            </option>
                            <option value="Debit" {{ $type == 'Debit' ? 'selected="selected"' : '' }}>@lang('index.debit')
                            </option>
                            <option value="Credit" {{ $type == 'Credit' ? 'selected="selected"' : '' }}>@lang('index.credit')
                            </option>
                        </select>
                    </div>
                </div>
                        ...
                        <tbody>
                            @if (isset($type) && $type == 'Credit')
                                @php
                                    $grandCredit = 0;
                                    $grandDebit = 0;
                                    $i = count($supplierDueReport);
                                @endphp

                                @if (isset($supplierDueReport))
                                    @foreach ($supplierDueReport as $key => $value)
                                        @php
                                            $totalDue = companySupplierBalance($value->id);
                                        @endphp
                                        ...
                                        @if ($totalDue > 0)
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-left">{{ getAmtCustom(abs($totalDue)) }} (@lang('index.credit'))
                                                </td>
                                            </tr>
                                        @endif
                                    @endforeach
                                    ...
                                    @foreach ($supplierDueReport as $key => $value)
                                        ...
                                        @if ($totalDue < 0)
                                            ...
                                            <tr>
                                                <td class="text-start">{{ $i-- }}</td>
                                                <td class="text-left">{{ $value->name }}</td>
                                                <td class="text-left">{{ getAmtCustom(abs($totalDue)) }}
                                                    (@lang('index.debit'))</td>
                                            </tr>
                                        @endif
                                    ...
                                    @foreach ($supplierDueReport as $key => $value)
                                        ...
                                        <tr>
                                            ...
                                            @elseif ($totalDue > 0)
                                                @php $grandCredit += $totalDue; @endphp
                                                <td class="text-left">{{ getAmtCustom(abs($totalDue)) }}
                                                    (@lang('index.credit'))</td>
                                            @else
                                                <td class="text-left">{{ getAmtCustom(abs($totalDue)) }}</td>
                                            @endif
                                        </tr>
                                    @endforeach
                                @endif
